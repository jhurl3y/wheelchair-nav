...
class App:
    ...
    def annotate_image(self, frame):
        ...
        # center the text
        p1 = (5, (width + text_size_right[1])/10)
        p2 = ((width - text_size_left[0] - 5), (width + text_size_right[1])/10)

        # then put the text itself
        cv2.putText(frame, text_right, p1, font_face, font_scale,
              (255, 255, 255), thickness, 8)
        cv2.putText(frame, text_left, p2, font_face, font_scale,
                (255, 255, 255), thickness, 8)
        return frame
    ...
    def run(self):
        while True:
            ...
            draw_str(vis, (500, 20), 'left count: %d' % self.left_mag) # give count      
            draw_str(vis, (20, 20), 'right count: %d' % self.right_mag) # give count 
